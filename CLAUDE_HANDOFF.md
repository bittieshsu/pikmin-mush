...
- `CONTROLLER_TOKEN`：Windows 維修 controller 專用 secret，不可與 `AGENT_TOKEN` 共用
...
2026-08-20 找到根因並非隨機：`RegisterMapObject` 只有在遊戲真的停在「即時地圖／
探索」畫面時才會觸發，GPS 覆寫（`SetOverride applied`）與地圖查詢往返
（`map query response received`）從安全提示彈窗、每日步數回顧、心情打卡、分享卡、
首頁儀表板送出時仍全部回報成功——沒有任何錯誤訊號能分辨「真的在地圖畫面」跟
「卡在其他畫面」。一般 app 內重啟（`restart_game_for_scan`，非完整裝置重開機）
幾乎都會直接落在首頁儀表板，不是對話框。

修法見 `phone_agent/agent.sh` 的 `wait_for_map_refresh()` fallback 分支：冷重啟
復原分三個固定時間點各觸發一次（不重複、不用 `KEYCODE_BACK`——這兩點都是繞了
彎路才確認的，見下）。**順序刻意把最先驗證可靠的動作排最前面**：`elapsed=8s`
先點首頁儀表板的探索／羅盤圖示（`MAP_VIEW_TAP_*`，實測穩定把畫面帶回地圖），
因為迴圈一偵測到 marker 就會立刻結束，排越前面的動作實際決定權越大；
`elapsed=20s`／`30s` 才輪到裝置真正重開機才需要的安全提示／繼續畫面座標
（`STARTUP_WARNING_Y` 等）與移動過快警告（`SPEED_WARNING_TAP_*`）。

繞的彎路，記下來避免重踩：① 一開始把所有座標「每 20 秒重複掃過一輪」，結果
發現固定座標點擊在畫面跟預期不同時會誤觸真實功能鍵（例如把既有的
`STARTUP_WARNING_Y` 打到儀表板的「完成N場探索」進度條，被帶去活動選單），
重複掃描只會放大這個風險、不會收斂。② 加了 `KEYCODE_BACK` 想收斂回上一層，
結果從儀表板（沒有更上層可退）按 BACK 直接把遊戲整個切到背景、跳到桌面
其他 App——比任何誤觸都更糟，已完全移除。最後定案是「不重複、不用 BACK、
順序按可靠度排列」，端對端重複驗證兩次穩定成功。

同時把 `MAP_REFRESH_TIMEOUT_SECONDS` 從舊預設 `0`（等於每點都直接重啟，不管
即時刷新有沒有機會成功）改成 `18`，讓即時刷新真的有機會先跑；重啟仍保留
作為安全網。
