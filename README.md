...
- 四台實體 Android Agent 可並行掃描；一般運作不依賴 Windows、USB、ADB、固定手機 IP 或同一區域網路。
- 雲端以逐點 lease 分派座標，同一工作內的 Agent 不會掃到相同 target；Agent 離線、停用或 lease 逾時後，座標會重新排隊。
- 每日 `07:30`、`19:30 Asia/Taipei` 自動換區。現有 4 時段輪替表涵蓋 67 個非台灣國家／區域包、427 個主要城市配置，四條當期路線互斥、各為 26–27 城，且不會沿用前一時段的國家包。
...
- 亞洲：日本、印度、韓國、泰國、馬來西亞、新加坡、印尼、菲律賓、越南
- 大洋洲：澳洲、紐西蘭
...
- 同一時段四條路線的國家包互不重疊；下一個 4 時段週期反轉分配順序。
...
| `agent_control_app/` | 手機端 Agent 即時 GPS／城市／掃描結果與暫停／恢復控制 App |
