...
export function parsePowerPause(input: unknown, now=Date.now()) {
  if(!input || typeof input!=='object') return null;
  const {id,paused_at,resumed_at,reason}=input as Record<string,unknown>;
  if(typeof id!=='string'||!/^[A-Za-z0-9._-]{6,100}$/.test(id)||typeof reason!=='string'||!POWER_REASONS.includes(reason)) return null;
  if(typeof paused_at!=='number'||!Number.isSafeInteger(paused_at)||paused_at<1_700_000_000_000||paused_at>now+300_000) return null;
  if(resumed_at!==null && (typeof resumed_at!=='number'||!Number.isSafeInteger(resumed_at)||resumed_at<paused_at||resumed_at>now+300_000))return null;
