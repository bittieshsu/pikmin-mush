...
test('retention checks coalesce within an isolate, keep D1 lease and retry transient errors', async()=>{
 ...
 new Script(ts.transpileModule(section,{compilerOptions:{module:ts.ModuleKind.CommonJS,target:ts.ScriptTarget.ES2022}}).outputText)
  .runInNewContext({exports,Date:Clock,runtime:()=>({DB:db}),retentionStatus:r=>r,
 ...
 now+=20000;await exports.runMushroomRetention();assert.equal(calls,3);
 now+=10001;fail=true;await assert.rejects(exports.runMushroomRetention());assert.equal(calls,4);
 fail=false;await exports.runMushroomRetention();assert.equal(calls,7);
